...
int synaptics_enable_gesture(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
	return synaptics_set_f12ctrl_data(rmi4_data, enable, 0);
}

//enable pdoze function
int synaptics_enable_pdoze(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
	unsigned char val[3];
	int retval;
...
//enable irq wakeup
int synaptics_enable_irqwake(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
	if (enable) {
...
ssize_t synaptics_rmi4_gesture_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", atomic_read(&syna_rmi4_data->syna_use_gesture));
}

ssize_t synaptics_rmi4_gesture_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
...
int synaptics_rmi4_crood_read(char *page, char **start, off_t off,
...
int synaptics_rmi4_pdoze_read(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len = 0;
...
int synaptics_rmi4_proc_double_tap_read(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	return sprintf(page, "%d\n", atomic_read(&syna_rmi4_data->double_tap_enable));
}

int synaptics_rmi4_proc_double_tap_write(struct file *filp, const char __user *buff,
...
int synaptics_rmi4_proc_camera_read(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	return sprintf(page, "%d\n", atomic_read(&syna_rmi4_data->camera_enable));
}

int synaptics_rmi4_proc_camera_write(struct file *filp, const char __user *buff,
		unsigned long len, void *data)
{
	int enable;
	char buf[2];
...
int synaptics_rmi4_proc_music_read(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	return sprintf(page, "%d\n", atomic_read(&syna_rmi4_data->music_enable));
}

int synaptics_rmi4_proc_music_write(struct file *filp, const char __user *buff,
		unsigned long len, void *data)
{
	int enable;
	char buf[2];
...
int synaptics_rmi4_proc_flashlight_read(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	return sprintf(page, "%d\n", atomic_read(&syna_rmi4_data->flashlight_enable));
}

int synaptics_rmi4_proc_flashlight_write(struct file *filp, const char __user *buff,
		unsigned long len, void *data)
{
	int enable;
	char buf[2];
...
int synaptics_rmi4_proc_smartcover_read(char *page, char **start, off_t off,
		int count, int *eof, void *data) {
	int len = 0;
	unsigned int enable;
...
int synaptics_rmi4_open_smartcover(void)
{
	int retval;
...
int synaptics_rmi4_close_smartcover(void)
{
	int retval;
	unsigned char val[10];
...
//smartcover proc write function
int synaptics_rmi4_proc_smartcover_write(struct file *filp, const char __user *buff,
		unsigned long len, void *data) {
	int retval;
	unsigned char bak;
	unsigned int enable;
...
int synaptics_rmi4_proc_glove_read(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len = 0;
	unsigned int enable;
...
int synaptics_rmi4_proc_glove_write(struct file *filp, const char __user *buff,
		unsigned long len, void *data)
{
	int retval;
	unsigned char val[1];
...
int synaptics_rmi4_proc_pdoze_read(char *page, char **start, off_t off,
		int count, int *eof, void *data) {
	int len = 0;
	unsigned int enable;
...
int synaptics_rmi4_proc_pdoze_write( struct file *filp, const char __user *buff,
		unsigned long len, void *data ) {
	unsigned int enable;
	char buf[2];
...
int keypad_enable_proc_read(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
...
int keypad_enable_proc_write(struct file *file, const char __user *buffer,
...
static int synaptics_rmi4_init_touchpanel_proc(void)
{
	...
	//glove mode inteface
	proc_entry = proc_create("glove_mode_enable", 0664, procdir);
	if (proc_entry) {
	...
	// double tap to wake
	proc_entry = proc_create("double_tap_enable", 0664, procdir);
	if (proc_entry) {
	...
	// wake to camera
	proc_entry = proc_create("camera_enable", 0664, procdir);
	if (proc_entry) {
	...
	// wake to music
	proc_entry = proc_create("music_enable", 0664, procdir);
	if (proc_entry) {
	...
	// wake to flashlight
	proc_entry = proc_create("flashlight_enable", 0664, procdir);
	if (proc_entry) {
	...
	proc_entry = proc_create("pdoze_mode_enable", 0664, procdir);
	if (proc_entry) {
	...
	//for smartcover
	proc_entry = proc_create("smartcover_mode_enable", 0664, procdir);
	if (proc_entry) {
	...
	//for pdoze status
	proc_entry = proc_create("pdozedetect", 0444, procdir);
	if (proc_entry) {
		proc_entry->read_proc = synaptics_rmi4_pdoze_read;
	}

	//for support tp2.0
	proc_entry = proc_create("coordinate", 0444, procdir);
	if (proc_entry) {
		proc_entry->read_proc = synaptics_rmi4_crood_read;
	}

	proc_entry = proc_create("keypad_enable", 0664, procdir);
	if (proc_entry) {
		...
		unsigned short f01_cmd_base_addr)
{
	...
	//power off device
	retval = regulator_disable(rmi4_data->regulator);
	msleep(30);
	rmi4_data->current_page = MASK_8BIT;
...
static int synaptics_rmi4_remove(struct i2c_client *client)
{
	...
	if (platform_data->regulator_en) {
		rc = regulator_disable(rmi4_data->regulator);
		regulator_put(rmi4_data->regulator);
	}
